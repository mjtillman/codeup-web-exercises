...
const vertClueGen = (rows, max) => {
  ...
  for (let i = 1; i < max - 1; i++) {
    ...
    if (C === 'X') {
      ...
      vertClues.push(countSquares([A, B, D, E, F]));
    }
  }
...
const outerClueGen = (minefield, corners, max) => {
  ...
  for (let i = 0; i < max; i++) {
    vertClues.push([minefield[i][0], minefield[i][1]]);
  }

  let leftClues = vertClueGen(vertClues, max);
  ...
  for (let i = 0; i < max; i++) {
    vertClues.push([minefield[i][max], minefield[i][max - 1]]);
  }

  let rightClues = vertClueGen(vertClues, max);
...
const setMines = (clues) => {

  clues = [].concat(...clues);
  let spaces = document.getElementsByClassName('field');
  ...
  clues.forEach((clue, i) => {
    switch (clue) {
      case '.':
        break;
      case 'X':
        spaces[i].style['background-color'] = 'red';
        break;
      default:
        spaces[i].innerText= `${clue}`;
    }
  });
